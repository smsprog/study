...
if [[ $1 == "clean" ]] ; then
	rm -rf ./src/AutoGenerated/*.java
	exit 0
fi

mkdir -p src/AutoGenerated

newFlName='src/AutoGenerated/AutoGenerated_new.java'

echo 'package AutoGenerated;' > $newFlName
echo 'import uobject.*;' >> $newFlName
echo 'import icommand.*;' >> $newFlName
echo 'public class AutoGenerated_new {' >> $newFlName
echo '	public static void IoCRegister() throws Exception {' >> $newFlName
echo '		ICommand cmd;' >> $newFlName

grep -i interface ./src/*/*.java | awk -F":" '{print $1}' | sort -r | while read fl ; do
	...
	classFlName='src/AutoGenerated/'$className'.java'
	...
	echo '		System.out.println("'$className': new()");' >>  $classFlName
	...
	echo '		cmd=IoC.Resolve("IoC.Register", "'$ifName'", (IFunction)((arr) -> new '$className'((UObject)arr[0]) ) ); cmd.exec();' >> $newFlName
